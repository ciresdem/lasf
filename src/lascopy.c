/*---------------------------------------------------------------
 *
 * lasreformat.c - las file library
 *
...
#define LASREFORMAT_VERSION "0.2.3"
...
usage() {
  fprintf(stderr, "\
lasreformat infile(s) [Options]\n\n\
  Reformat the given LAS file.  The default will output a \n\
  file version 1.3 with point record version 3.\n\n\
\
  infile(s)\tThe input LAS file(s) \n\
\n\
 Options:\n\
  -f, --format\t\tThe output LAS file format, \n\
  -p, --pointformat\tThe output LAS file point format, \n\
\n\
     Current Formats:\n\n\
   0: LAS Format 1.0\n\
   1: LAS Format 1.1\n\
   ...
   4: LAS Format 1.4\n\
\n\
     Current Point Formats:\n\n\
   0: LAS Point Format 0\n\
   1: LAS Point Format 1\n\
   2: LAS Point Format 2\n\
   3: LAS Point Format 3\n\
   4: LAS Point Format 4\n\
   5: LAS Point Format 5\n\
   6: LAS Point Format 6\n\
\n\
     ...
     char **argv;
{
  ...
  if (version_flag) lasf_print_version("lasreformat", LASREFORMAT_VERSION);
  ...
  while (optind < argc) {
	...
	lasf_print_error(status, "lasreformat");
	exit(-1);
      }
	...
	exit(-1);
      ...
      fprintf(stderr, "lasreformat: Working on file %s\n", fn);
	...
	exit(-1);
      ...
      status = lasf_put_header_gensoftware(&lasfhid_out, "lasreformat");
      if (status != lasf_NOERR) {
	lasf_print_error(status, "lasreformat");
	exit(-1);
      }
